...
# --- Helper function for quick card creation in tests ---
...
class TestSolitaireGame(unittest.TestCase):
    ...
    def test_initial_setup(self):
        ...
        # Tableau setup (1 to 7 cards, top card face up)
    ...
    def test_draw_from_stock_and_recycle(self):
        ...
        self.assertTrue(self.game.waste.cards[0].face_up)
        
        # 2. Exhaust the stock (draw remaining 23)
        for _ in range(23):
            self.game.draw_from_stock()
        self.assertEqual(len(self.game.stock.cards), 0)
        self.assertEqual(len(self.game.waste.cards), 24)
        
        # 3. Recycle the waste (draw again)
        self.game.draw_from_stock() 
        self.assertEqual(len(self.game.waste.cards), 1) # 1 card in waste
        self.assertEqual(len(self.game.stock.cards), 23) # 23 cards in stock (flipped face-down)
        self.assertFalse(self.game.stock.cards[0].face_up)
    ...
    def test_can_place_tableau_rules(self):
        ...
        # Invalid: Not one rank lower (King on Queen)
        red_king = make_card('king', 'diamonds')
        black_queen_pile = Pile([make_card('queen', 'spades')])
        self.assertFalse(self.game.can_place_tableau(red_king, black_queen_pile))

        # Valid: Any card on an empty pile (per game.py logic)
        self.assertTrue(self.game.can_place_tableau(red_king, Pile()))
